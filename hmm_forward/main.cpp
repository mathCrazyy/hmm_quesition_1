...
int main(){
	...
	*/
	...
	//------全排列------
	string watch_value_test[4] = { "1", "2", "3","4" };
	vector<string> watch_pos_test(watch_value_test, watch_value_test + 3);
	vector<string> route_test;
	//假设长度为6的观测序列，状态取值为1，2，3，4.
	vector<string> all_route_test = get_group(watch_pos_test, route_test, 0, 6);

	for (int i = 0; i < all_route_test.size(); i++){
		cout << all_route_test[i] << endl;
	}
	...
	for (int i = 0; i < transition_matrix.size(); i++){//第一个路劲的取值，由初始pi决定。
		cout<<"状态转换" << i << "-->" << route_int[i] - 1 << endl;
	...
	for (int route_index = 0; route_index < all_route.size(); route_index++){
		...
		//for (int i = 0; i < transition_matrix.size(); i++){//第一个路劲的取值，由初始pi决定。
		//	cout << "  - " << i <<"--"<< route_int[i] - 1 <<"--"<< transition_matrix[i][route_int[i] - 1] << endl;
		//	transition_value *= transition_matrix[i][route_int[i] - 1];
		//	
		//}
		float pi_transition_value = pi[route_int[0] - 1] * transition_value;//转移矩阵的概率.
